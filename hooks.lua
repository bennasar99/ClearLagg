function OnTick(TimeDelta)
    ...
    if (Time == TimeUntilRemove) then
        ...
        cRoot:Get():BroadcastChat(cChatColor.Red.. "[ClearLagg] Removed "..Entities.." entities")
        Entities = 0
    elseif (Time == ToRemove) then
        cRoot:Get():BroadcastChat(cChatColor.Red.. "[ClearLagg] Ground items will be removed in 1 minute!")
        Time = Time + 1
    else
        Time = Time + 1
    end
